...
public class UserController {
    ...
    private final com.example.Qnect.service.CommentService commentService;

    public UserController(UserService userService, PostService postService,
            com.example.Qnect.service.CommentService commentService) {
    ...
    public ResponseEntity<com.example.Qnect.dto.PageDto<FeedPostDto>> getUserQuestions(
            @PathVariable Long id,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        com.example.Qnect.dto.PageDto<FeedPostDto> questions = postService.getUserQuestions(id, page, size);
        return ResponseEntity.ok(questions);
    }

    @GetMapping("/{id}/posts-by-type")
    public ResponseEntity<com.example.Qnect.dto.PageDto<FeedPostDto>> getUserPostsByType(
            @PathVariable Long id,
            @RequestParam String type,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        try {
            com.example.Qnect.models.PostType postType = com.example.Qnect.models.PostType
                    .valueOf(type.toUpperCase());
            com.example.Qnect.dto.PageDto<FeedPostDto> posts = postService.getUserPostsByType(id, postType, page,
                    size);
    ...
    @GetMapping("/search")
    public ResponseEntity<com.example.Qnect.dto.PageDto<UserProfileDto>> searchUsers(
            @RequestParam String query,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        Page<UserProfileDto> users = userService.searchUsers(query, page, size);
        return ResponseEntity.ok(new com.example.Qnect.dto.PageDto<>(users));
    }

    @GetMapping("/{id}/comments")
    public ResponseEntity<com.example.Qnect.dto.PageDto<com.example.Qnect.dto.PostCommentDto>> getUserComments(
            @PathVariable Long id,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            Authentication authentication) {
