...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("SELECT p FROM Post p JOIN FETCH p.author WHERE p.author.userId <> :userId AND p.type <> com.example.Qnect.models.PostType.NEWS_DISCUSSION ORDER BY p.createdAt DESC")
    List<Post> findRecentExcludingUser(@Param("userId") Long userId, Pageable pageable);

    // Recent posts - Cursor Load
    @Query("SELECT p FROM Post p JOIN FETCH p.author WHERE p.author.userId <> :userId AND p.type <> com.example.Qnect.models.PostType.NEWS_DISCUSSION AND p.createdAt < :cursor ORDER BY p.createdAt DESC")
    ...
    @Query("SELECT p FROM Post p JOIN FETCH p.author WHERE p.type = com.example.Qnect.models.PostType.QUESTION AND p.author.userId <> :userId AND p.commentsCount = 0 ORDER BY p.createdAt DESC")
    List<Post> findUnansweredExcludingUser(@Param("userId") Long userId, Pageable pageable);

    // Unanswered questions - Cursor Load
    @Query("SELECT p FROM Post p JOIN FETCH p.author WHERE p.type = com.example.Qnect.models.PostType.QUESTION AND p.author.userId <> :userId AND p.commentsCount = 0 AND p.createdAt < :cursor ORDER BY p.createdAt DESC")
    ...
    @Query("SELECT p FROM Post p JOIN FETCH p.author WHERE p.author.userId <> :userId AND p.type <> com.example.Qnect.models.PostType.NEWS_DISCUSSION ORDER BY p.baseHotnessScore DESC, p.id DESC")
    ...
    @Query("SELECT p FROM Post p JOIN FETCH p.author WHERE p.author.userId <> :userId AND p.type <> com.example.Qnect.models.PostType.NEWS_DISCUSSION AND (p.baseHotnessScore < :cursorScore OR (p.baseHotnessScore = :cursorScore AND p.id < :cursorId)) ORDER BY p.baseHotnessScore DESC, p.id DESC")
