package com.example.Qnect.service;
...
import com.example.Qnect.models.PostType;
import com.example.Qnect.models.User;
import com.example.Qnect.repository.PostRepository;
import com.example.Qnect.repository.UserRepository;
...
public class PostService {
    ...
    private final com.example.Qnect.repository.PostViewRepository postViewRepository;
    private final com.example.Qnect.repository.VoteRepository voteRepository;
    private final com.example.Qnect.repository.CommentRepository commentRepository;
    private final com.example.Qnect.repository.BookmarkRepository bookmarkRepository;
    private final com.example.Qnect.repository.TopicRepository topicRepository;
    private final FileStorageService fileStorageService;

    private final org.springframework.cache.CacheManager cacheManager;

    public PostService(PostRepository postRepository, UserRepository userRepository,
                       com.example.Qnect.repository.PostViewRepository postViewRepository,
                       com.example.Qnect.repository.VoteRepository voteRepository,
                       com.example.Qnect.repository.CommentRepository commentRepository,
                       com.example.Qnect.repository.BookmarkRepository bookmarkRepository,
                       com.example.Qnect.repository.TopicRepository topicRepository,
    ...
    public Post createPost(Long authorId, CreatePostRequest request) {
        ...
        if (request.getTags() != null && !request.getTags().isEmpty()) {
            java.util.Set<com.example.Qnect.models.Topic> topics = new java.util.HashSet<>();
            ...
            List<com.example.Qnect.models.Topic> existingTopics = topicRepository.findByNameInIgnoreCase(validLowerNames);
            topics.addAll(existingTopics);

            // Map of lower-case name -> Topic for easy lookup
            java.util.Map<String, com.example.Qnect.models.Topic> existingTopicMap = existingTopics.stream()
            ...
            for (String name : tagNames) {
                if (!existingTopicMap.containsKey(name.toLowerCase())) {
                     com.example.Qnect.models.Topic newTopic = com.example.Qnect.models.Topic.builder()
                    ...
                    com.example.Qnect.models.Topic t = existingTopicMap.get(name.toLowerCase());
    ...
    public void deletePost(Long postId, Long userId) {
        ...
        try {
            ...
            voteRepository.deleteByEntityTypeAndEntityId(com.example.Qnect.models.Vote.EntityType.QUESTION, postId);
            
             // Delete votes for comments (if any)
            if (post.getComments() != null) {
                for (com.example.Qnect.models.Comment comment : post.getComments()) {
                     voteRepository.deleteByEntityTypeAndEntityId(com.example.Qnect.models.Vote.EntityType.COMMENT, comment.getId());
                }
            }
    ...
    @Transactional(readOnly = true)
    public com.example.Qnect.dto.CursorPageDto<FeedPostDto> getFeedForUser(Long userId, FeedTab tab, String cursor, int size) {
        ...
        com.example.Qnect.dto.CursorPageDto<FeedPostDto> response = new com.example.Qnect.dto.CursorPageDto<>();
    ...
    public com.example.Qnect.dto.PageDto<FeedPostDto> getUserQuestions(Long userId, int page, int size) {
        ...
        return new com.example.Qnect.dto.PageDto<>(pageResult);
    }
    ...
    public com.example.Qnect.dto.PageDto<FeedPostDto> getUserPostsByType(Long userId, com.example.Qnect.models.PostType type, int page, int size) {
        User author = userRepository.findById(userId)
                .orElseThrow(() -> new RuntimeException("User not found"));
        Pageable pageable = PageRequest.of(page, size, Sort.by(Sort.Direction.DESC, "createdAt"));
    ...
    public Page<FeedPostDto> getPostsByTopic(Long topicId, Long userId, int page, int size) {
        ...
        if (posts.isEmpty() && page == 0) {
            com.example.Qnect.models.Topic topic = topicRepository.findById(topicId).orElse(null);
            if (topic != null) {
    ...
    public FeedPostDto getPostByIdAndIncrementViews(Long postId, Long userId) {
        ...
        if (userId != null) {
            ...
            if (!alreadyViewed) {
                com.example.Qnect.models.PostView view = com.example.Qnect.models.PostView.builder()
                        .user(user)
                        .post(post)
    ...
    dto.setTags(post.getTags() != null ? new java.util.ArrayList<>(post.getTags()) : null);
        ...
        if (userId != null) {
             java.util.Optional<com.example.Qnect.models.Vote> vote = voteRepository.findByUserAndEntityTypeAndEntityId(
                     userRepository.getReferenceById(userId), 
                     com.example.Qnect.models.Vote.EntityType.QUESTION, 
                     post.getId());
    ...
    private Page<FeedPostDto> convertToFeedPostDtoBulk(Page<Post> posts, Long userId) {
        ...
        if (userId != null) {
            ...
            // Votes: single query
            java.util.List<com.example.Qnect.models.Vote> votes = voteRepository.findAllByUserAndEntityTypeAndEntityIdIn(
                    user, com.example.Qnect.models.Vote.EntityType.QUESTION, postIds);
            
            voteMap = votes.stream()
                    .collect(java.util.stream.Collectors.toMap(
                            com.example.Qnect.models.Vote::getEntityId,
                            v -> v.getVoteType().name(),
                            (a, b) -> a));

            // Bookmarks: single query
            java.util.List<com.example.Qnect.models.Bookmark> bookmarks = 
