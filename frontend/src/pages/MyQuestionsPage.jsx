...
import { getAuthToken, userApi, questionApi, voteApi } from '../api';
...
export default function MyQuestionsPage() {
  ...
  const load = async () => {
    try {
      ...
      setError(e?.message || 'Failed to load your questions');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeletePost = async (postId) => {
    ...
    try {
      ...
      setQuestions(prev => prev.filter(q => q.id !== postId));
    } catch (e) {
      console.error(e);
        ...
        <main className="flex-1 md:ml-64 p-6 max-w-5xl">
          ...
          {loading ? (
            ...
            <div className="bg-white p-12 rounded-lg border border-gray-200 text-center">
              ...
              <p className="text-gray-500 mb-4">You haven't created any posts.</p>
              <button
